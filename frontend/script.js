  ...
  try {
    ...
    scores.forEach((score) => {
      ...
      checkButton.addEventListener('click', (e) => {
		e.stopPropagation();
		const user = getURLParameter('user'); // user 값을 전달하기 위한 변수
        ...
        window.location.href = `/chklist.html?user=${encodeURIComponent(user)}&gp=${encodeURIComponent(gp)}&nm=${encodeURIComponent(nm)}&chk=${encodeURIComponent(chk)}`;
      });
...
window.onload = async () => {
  ...
  //addClickEventToTotalScores();  // 총점 클릭 이벤트 추가, loadScores() 호출시 처리로 변경
  toggleSubmitButton();
